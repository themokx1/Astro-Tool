...
/// finished output from its own filename. The two lineage tables that could
/// once have carried it had no writer anywhere in the product and were
/// dropped in schema v8 (W4-6, owner decision) -- see `MetadataSchema.
/// versionEightSQL`'s own note -- so a lineage panel here would be a panel
/// that can never fill.
