...
public struct GlossaryView: View {
    ...
    public static let terms: [Term] = [
        ...
        // W7-F item 1 (2026-08-18 expert audit, workflow #4): no standalone
        // entry existed before -- "meridian flip" was only ever mentioned
        // parenthetically inside "Culmination" above. Added because the
        // Home/Planning culmination annotations now name this term directly.
        Term(name: "Meridian flip", definition: "A German equatorial mount (GEM) must rotate 180 degrees around both its axes when a target crosses the meridian, so the counterweight doesn't collide with the tripod/pier -- a real interruption in an imaging session (re-pointing, often refocusing and re-framing). Alt-azimuth and fork mounts have no meridian to flip around, so this doesn't apply to them. This app has no mount-type data, so it can only flag that a flip is LIKELY for a GEM mount, never confirm one."),
